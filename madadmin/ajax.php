...
} elseif ($act == "get_attr")//获取商品属性
{
	...
	EchoJson($arr, "error");
} elseif ($act == "get_shipping_region") {
	...
	if (isset($_POST['id'])) {
		...
		if (!empty($id)) {
			...
			$row = $db->getAllFromTable("region", "where parent_id=$id");
			EchoJson($arr, "ok", $row);
	...
	EchoJson($arr, "error");
} elseif ($act == "shipping_area_region_init") {
	if (isset($_POST['id'])) {
		$id = intval($_POST["id"]);

		if (!empty($id)) {

			$sql = "select * from area_region as a left join region as r on a.region_id=r.id where a.shipping_area_id=$id";
			$row = $db->getAll($sql);
			EchoJson($arr, "ok", $row);
