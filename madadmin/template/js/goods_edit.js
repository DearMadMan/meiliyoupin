	...
	$(this).click(function(){
		if(confirm("确定要删除该图片吗？"))
		{
			...
			$.ajax({
				...
				dataType:"json",
				url:"ajax.php",
				data:"act=goods_edit_gallery_delete&path="+li.attr("data")+"&gallery_id="+li.attr("gallery"),
				cache:false,
...
function AddAttr()
{
	 ...
	 $.ajax({
	 	...
	 	url:"ajax.php",
	 	cache:false,
	 	data:"act=goods_edit_addattr&goods_id="+goods_id+"&attr_name="+attr_name.val()+"&attr_price="+attr_price.val(),
	 	success:function(msg){
	 		if(msg.error=="ok")
	 		{
...
function EditToSave(obj)
{
	...
	$.ajax({
		...
		cache:false,
		url:"ajax.php",
		data:"act=goods_edit_edit_attr&attr_key="+tr.attr("data")+"&attr_name="+attr_name.val()+"&attr_price="+attr_price.val(),
		success:function(msg){
			if(msg.error=="ok")
			{
...
function DeleteAttr(obj)
{
	...
	$.ajax({
		...
		url:"ajax.php",
		cache:false,
		data:"act=goods_edit_delete_attr&attr_key="+tr.attr("data"),
		success:function(msg){
			if(msg.error=="ok")
			{
				tr.css("background","#D8FDD8");
