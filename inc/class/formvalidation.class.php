...
class formvalidation {
	 ...
	 * const 常量
	 */
	const VA_EMPTY                           = "VA_EMPTY";
	const VA_CAN_EMPTY                       = "VA_CAN_EMPTY";
	const VA_EMAIL                           = "VA_EMAIL";
	const VA_NUMBER                          = "VA_NUMBER";
	const VA_COUNT                           = "VA_COUNT";
	const VA_COUNT_GT                        = "VA_COUNT_GT";
	const VA_BETWEEN                         = "VA_BETWEEN";
		...
		switch ($type) {
			...
			case self::VA_BETWEEN:
				...
				if (strlen($targetValue) > $msg[0] && strlen($targetValue) < $msg[1]) {
					...
					$is_true = false;
					$this->SetErrorMsg(self::VA_BETWEEN, "字符串位数应大于 ".$msg[0]."并且小于 ".$msg[1], $key);
				}
				break;
			case self::VA_MOBILE:
